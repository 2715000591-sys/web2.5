...
- Worker Version ID：`e74435d5-b7cd-498b-b76f-c7d83c90adef`
- 当前线上版本包含“测试一次 AI 接入”入口、DeepSeek JSON 标签提示补强、扩展侧 AI 排队保护、保存 AI 设置时不再误清空已有 Key 的修复、2026-05-03 上线并加大的 AI 老师抽查机制、2026-05-03 12:56 前置老师复核、2026-05-04 01:24 低风险等待 AI 时先显示、2026-05-04 01:59 队列提速 / pending 重试 / 已知命中先返回且老师后台复核、2026-05-04 09:38 底部整理卡片和官网控制台统一“后台/手动记录”真实口径、2026-05-04 09:57 底部查看列表缓存修复，以及 2026-05-03 10:05 / 11:21 / 11:40 / 14:12 / 23:45 / 2026-05-04 00:37 补入的短口号、埃及符号壳、`寻男大固泡`、真人/DD 诱导昵称、中英混合短口号、花体/上标英文标签和装饰壳短口号证据类型。
- 2026-05-04 10:34 本机 Safari 和公网下载包都已更新到 `BUILD_ID=2026-05-04-1034` / `extensionVersion=0.1.74`，把旧手动教学 / 历史命中合并显示为“后台学习库下沉”，把等待后台判断单独显示为“后台复审中”，并把后台返回的英文原因短句转成中文概括展示。这次没有改 AI 调度和 Worker 审核逻辑；用户完成 Cloudflare 登录验证后已补发公网，后续同版本重发一次，线上 AI/Worker 锚点为上一行的 Version ID。
...
如果改动扩展侧 AI 调度、候选过滤、页面采集或 BUILD_ID，需要重建并替换本机 Safari App。2026-05-04 已替换 `/Applications/web2.5.app` 到 `BUILD_ID = 2026-05-04-1034`。
...
- 记忆命中直接返回结果，控制台归入 `后台学习库下沉`，不消耗外部 API。
