...
2026-05-01 后续排查发现：线上 DeepSeek 配置的接口地址和模型还在，但 API Key 已被一次普通保存误清空。原因是 Worker 接口把“请求里没有带 apiKey”错误当成“清空 apiKey”。已修复并部署到 Worker Version ID `e7793119-2da3-4cde-9c03-65c5e6d7e08a`：以后保存开关、模型或提示词时，如果没有提交新的 Key，会保留原来的加密 Key。已经被清空的 Key 无法从数据库恢复，用户需要重新输入一次 Key 才能恢复真实 AI 调用。

## 1. 当前结论

现在已经不是只能接 DeepSeek。

截至 2026-05-01，线上 Worker 已经支持通用大模型兼容接入。开发者账号曾经接入 DeepSeek 并通过真实调用测试；当前接口地址和模型仍保留，但 Key 因上面的保存 bug 已为空，需要重新填一次：

- 兼容接口地址：`https://api.deepseek.com`
- 模型：`deepseek-v4-flash`
- API Key：当前线上记录为空；修复已上线，重新填一次后不会再被普通保存误清。
...
- DeepSeek `deepseek-v4-flash` 曾接入线上开发者账号，并通过真实调用测试；当前需重新填一次 Key。
...
- 真实 DeepSeek API Key 曾接入并做过小样本真实调用测试；当前 Key 已被误清空，需要重新输入一次。
