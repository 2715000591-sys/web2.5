...
1. 数据库和 AI 记忆先扫一遍。
2. 数据库明确知道是垃圾时，页面先直接撤掉，不必等外接 AI；AI 老师如需复核，应在后台补课，不阻塞页面处理。
3. 数据库不知道、但候选可疑时，尽快交给 AI 判断。
4. AI 判定垃圾后，立即撤掉，并把这次判断写入 `reply_ai_results`、`moderation_sample_labels`、`reply_ai_memory` 和候选规则，让数据库下次自己会。
5. AI 判定没问题时，内容保持或恢复可见；用户恢复误判只作为纠错和抑制证据。
6. 临时隐藏不是主路线，只能用于数据库已确认或极高风险内容。后续优化要尽量减少“先隐藏再等 AI 证明没事”的体验。
...
数据库 / AI 记忆 / 已知候选规则快速扫
  ↓
已知垃圾：页面立刻撤掉，AI 老师后台补课
未知但可疑：立刻送 AI，低风险等待时保持可见
极高风险：可临时隐藏，等 AI 最终判断
  ↓
AI hide：页面撤掉，写入结果、标注、记忆和候选
AI allow：页面保留，必要时清掉等待状态
```
...
2026-05-04 追加口径：`reply_ai_results.status=pending / 等待 AI 判断` 只是“老师还没判完”，不能直接等同于垃圾。扩展侧从 `BUILD_ID=2026-05-04-0124` 起把“送 AI 老师判断”和“等待期间先隐藏”分开：低风险候选会继续送 AI 并留下后续学习证据，但等待时先保持可见；只有强风险或候选分数达到 3 的内容才临时隐藏。用户手动 `恢复` 这类 pending/误藏内容时，仍只作为纠错和抑制证据，不应反向训练成公共放行规则。

2026-05-04 01:59 追加速度口径：从 `BUILD_ID=2026-05-04-0159` 起，临时隐藏进一步收紧到极高风险或候选分数达到 6；页面队列 180ms 内发出，作者资料取证最多等 1.2 秒，AI 批量缺项/失败会自动重试。Worker 命中数据库、AI 记忆或已知规则时先立刻返回页面，AI 老师复核放到后台继续写标注、记忆和候选；未知可疑内容才等外接 AI 的最终判断。当前 DeepSeek 不能看图时，头像证据候选继续合在文本批量里判断，不拆成慢的单图调用。本轮没有 schema 变更、没有清理或删除 D1 数据。

2026-05-04 11:59 追加 pending 补判口径：如果真实页面样本已经写入 `reply_ai_items` 和 `reply_ai_results.status=pending`，但 `model` 仍为空，不能向用户说“AI 已经看过但放过了”。这表示后台收到了样本，还没有可靠留下最终 AI 判断。新版从 `BUILD_ID=2026-05-04-1159` 起把前端和 Worker 回复 AI 单批从 8 条缩到 4 条，前端等待后台结果放宽到 55 秒，Worker 写入 pending 后会在后台安排一次补判，页面收到 `failed / AI 批量结果不完整` 会重新排队再试，后台补判也会捞这类失败项，减少浏览器请求中途断开、批量返回不完整或 AI 慢时留下长期 pending/failed。用户发来的漏网截图仍应先当作 AI/数据库教学链路诊断素材：查是否进 AI、是否 pending、是否 AI 放过、是否数据库记忆命中；不要直接把截图里的具体词硬写进数据库。
...
- 2026-05-03 10:46 再次提高强度：本地批量上限 12、等待 650ms、最小间隔 750ms、基础可疑分 1、老师复核分 3、会话缓存 360。
- 高风险候选会带 `teacher_review_requested` 证据标签；风险昵称、头像取证、关系诱导、垃圾模板等证据也会更积极地进入老师复核链路。
- Worker 仍按静态规则、AI 记忆、数据库候选规则的顺序先拦住明显垃圾。
- 当数据库候选规则已经命中，并且候选带老师复核标签或头像/风险昵称/关系诱导等证据时，Worker 每批最多额外抽 8 条调用已配置的 DeepSeek。
...
## 2026-05-03 真实写入和探针区分

用户再次追问“探针和真实写入是否已经区分”。后续必须按这几个层次判断：

- 开发者探针：用于看一条样本会走哪条路线，默认不写数据库；不能用它证明真实 X 页面已经学习。
- `reply_ai_items`：真实 Safari 页面已经把可疑回复写到云端，说明监听和写入已发生。
- `reply_ai_results.status=pending`：真实样本已收到，正在等待 AI 判断；不是最终结论。
- `reply_ai_results.status=ready`：AI、数据库、记忆库或规则已经给出最终可用结论。

2026-05-03 22:46 只读 D1 发现当前设备真实页面 2026-05-03 09:00 UTC 后 16 条 `reply_ai_items` 中有 14 条没有对应 `reply_ai_results`。处理方向：把本地和 Worker 单批候选量从 16 降到 8，老师复核预算从 16 降到 8，并在云端收到真实样本后先写 `pending` 结果，避免“写入了但状态空白”。这不是让 AI 少参与，而是让 AI 判断更稳定、更可追踪。
- 本轮没有 schema 变更、没有 D1 清理、没有删除生产数据。

2026-05-03 用户再次强调 AI 参与强度应更大：如果主要靠用户本人不断手动冲走，数据库训练效率太低。本轮已经按这个方向开大代码强度；后续仍应让 AI 老师主动多看高风险和边界样本，尤其是头像/图片有招嫖引流证据、随机数字账号、低信息口号、emoji 噪音、上下文脱节、数据库已命中但仍值得复核、或反复出现的新模式。AI 高置信结论要继续写入 `reply_ai_results`、`moderation_sample_labels`、`reply_ai_memory`，并刷新候选规则。普通正常回复仍不全量进 AI，但不要把节省 token 放在训练效率之上。
...
2026-05-03 用户进一步明确：他们发来的任何漏网内容，目的都是优化 AI 配置、证据输入和 AI 老师学习链路，不是让 Codex 直接在本地硬写短语规则。正确闭环应是：截图/样本进入诊断 -> AI 老师用昵称、handle、正文、头像、上下文和历史证据判断 -> AI 高置信结论写入 `reply_ai_results`、`moderation_sample_labels`、`reply_ai_memory` 和候选规则 -> 再由 AI/数据库证据或开发者明确确认把可复用模式写回本地和 Worker。除非用户明确要求或已有足够 AI/数据库证据，不要把单张截图里的文字直接固化到本地规则。

2026-05-03 11:21 追加漏网诊断：`灵魂不负相逢`、`余生静候灵魂归`、`静待世间共鸣人`、`远离三观不合人`、`同频方可共余生`、`静候岁月遇知音`、`随缘静待同频` 这批不是扫描次数不够，真实页已经扫完 53 条。漏网原因是证据类型缺失：X 正文只暴露埃及象形装饰符号壳和空洞中文口号，旧规则没覆盖这段符号，也没覆盖这组抽象交友空话词。处理方向是补“证据类型”和本地/Worker 同构规则，让以后同类候选能被基础层、数据库和 AI 老师看懂；不是把单个截图账号或单次用户反馈升级成公共规则。

2026-05-03 11:40 追加漏网诊断：`孙甜甜❤️寻男大固泡 @MonaKristi9125` 这种名字已经应视为强风险昵称，正文纯 emoji 噪音只是加重证据。旧规则漏掉是因为风险词只覆盖 `找固定泡友/炮友/固炮`，没有覆盖 `寻男 + 固泡` 缩写；并且 X 把 emoji 正文渲染成图片 alt，旧正文提取只读普通文字时没有把 emoji 放进样本。处理方向仍是补证据输入和同构风险词：让样本里保留 emoji alt，并让本地/Worker 都认识 `寻男/寻女/固泡/泡友/炮友/性友`。这不改变公共规则升级原则，也不允许单用户重复冲走直接变公共规则。
...
2026-05-04 00:37 用户截图补充一类“AI 没审核到”的真实原因：候选没进队列。`ᴰᵉˡᵘˢⁱᵒⁿ`、`ᴴᵘˢʰ` 这类上标/花体英文需要先正规化再抽重复英文标签；埃及装饰符号壳不能算进泛化短口号长度，否则短中文空话会被误判为太长。处理方向仍是补证据抽取和候选键同构，不是把截图里的具体词写死。
