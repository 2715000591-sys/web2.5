...
- 最终主路线必须朝这个方向收敛：数据库 / AI 记忆先扫；数据库明确知道是垃圾就立刻撤掉；数据库不知道时，尽快交给 AI 判断；AI 判定垃圾就撤掉并写回标注、记忆和数据库候选；AI 判定没问题就留在页面上。
- `临时隐藏` 不是最终主流程，只是极高风险内容在等待 AI 时的安全桥。后续优化要继续减少临时隐藏，让更多边界内容保持可见，等 AI 真正判定后再处理。
- 用户希望看到的是“数据库瞬间处理已知问题，AI 尽快处理未知问题”，不是“先藏起来再慢慢证明它没事”。这个体验目标优先于为了省 token 或省实现步骤而扩大预隐藏。
...
- 2026-05-04 11:59 起，针对真实漏网样本 `全网唯一真实社区 / 线下直接对接 / emoji 空回复`，不要把截图词直接写入数据库；稳定做法是把它抽象为“招揽昵称或线下对接昵称 + 随机账号 + 空 emoji / 低信息回复 + 与主帖无关”。本地和 Worker 已同步补通用风险昵称证据，前端和 Worker 单批 AI 从 8 条缩到 4 条，前端等待后台结果放宽到 55 秒，Worker 写入 pending 后会安排后台补判，防止 `pending / 等待后台判断 / model 空` 长期挂住。页面收到 `failed / AI 批量结果不完整` 会重新排队再试，后台补判也会捞这类失败项。线上探针已确认同类样本会真实调用 DeepSeek，并返回 `ai / ready / hide / high`；探针不写数据库。
- AI 第一次高置信隐藏后，云端数据库会作为“AI 记忆本”复用结果
- 本地规则继续负责采集、风险信号、立即安全下沉和排队排序；普通正常回复不进 AI 队列。

2026-05-03 17:15 用户阶段验收：这一版真实刷起来已经“屏蔽挺好，没有明显漏过”，可以先作为一个阶段稳定点。后续主要问题不是继续无脑加严，而是处理少量误杀：粗话、反驳、吐槽、讽刺如果和原帖上下文强相关，应尽量放过。典型样本：原帖核心是“没钱/有钱导致过得不好”，回复 `你穷怕了` 是相关正常评论，用户已恢复；系统已记录 `manual_allow` 和样本 allow 标注。未来改 AI 提示词或候选入口时，必须让 `mainPostText` 的语义关联更有分量。
...
- 低信息垃圾回复 + 风险账号画像
  ...
  - 随机英文数字账号发“英文标签 + 中文空洞短句 + 同一英文标签 + emoji”的中英混合批量口号
- 明显关系 / 情色搭讪
  - `哥哥我想要`
  - `有哥哥找下吗`
...
- `bilingual-short-slogan-lure-account`
...
- 本地 Safari 扩展构建：
  ...
  - 2026-05-04 11:59 已修用户指出的“明显线下对接/真实社区诱导账号进了后台但 pending 卡住”问题。新版同步本地和 Worker：补通用线下对接/真实社区/资源牵线类风险昵称证据，AI 单批从 8 条缩到 4 条，前端等待后台结果放宽到 55 秒，Worker pending 后 12 秒安排补判；页面收到批量结果不完整失败会重新排队再试。线上探针 `🔥全网唯一真实社区🥹线下直接对接 @belanich34344 / ⚡🤩💨🥳` 真实调用 DeepSeek，返回 `ai / ready / hide / high`，理由为诱导昵称、可疑账号、emoji 空回复且与主帖无关；`Database writes: no`。本轮没有改数据库结构、没有删除 D1 数据、没有新增付费 AI 能力。
  - 2026-05-04 10:34 已修底部整理卡片的数据真实性口径：旧手动教学 / 历史命中统一显示为“后台学习库下沉”，不再单独显示“手动记录下沉”；`ai-pending` 单独计为“后台复审中”，避免把等待后台判断的候选误算进“本机自动下沉”；后台返回英文原因短句时，底部卡片会展示中文概括。真实 Safari 页 `https://x.com/fadai202202/status/2051011398134911475` 验证 `build=2026-05-04-1034`、`stage=scan:done`、`articles=13`，底部显示 `后台已判断 6 条，后台自动下沉 1 条，后台学习库下沉 3 条`，展开 / 收起 / 等待 4 秒后 4 张底部卡片稳定存在且按钮不变灰。本轮没有改筛选阈值、数据库结构或 D1 数据，也没有新增付费 AI 能力。
  - 2026-05-04 09:38 已继续统一控制台、底部整理卡片和后台原因短句的用户文案：对用户统一叫“后台直接下沉 / 后台学习库下沉 / 后台已判断”，不再让用户区分 AI 后台和数据库后台。本轮只改文案和版本号，不改筛选阈值、数据库结构或真实数据。
  - 2026-05-04 09:18 已修底部整理卡片的数据真实性口径：线上只读核验没有新的 `manual_hide`，所以页面不能把历史/本机手动记录写成“你刚标记”。新版统一为“后台已判断 X 条（下沉 Y 条）/ 本机自动下沉 / 手动记录下沉 / 后台学习库下沉”，AI 判断和数据库学习库在用户界面统一叫后台；历史手动命中使用 `history` 来源，不再计入“本次手动”。本轮只改文案和来源分类，不改筛选阈值、数据库结构或真实数据。
...
- 云端 Cloudflare Worker：
  ...
  - 2026-05-04 11:59 已部署 `BUILD_ID=2026-05-04-1159` / `extensionVersion=0.1.76`。本轮重点是防止明显高风险候选进入后台后长期停在 `pending`，并把线下对接/真实社区类昵称作为通用风险证据交给 AI 老师判断；不是把用户截图词写死到数据库。公网首页、控制台和 `/downloads/latest.json` 均确认可访问，下载清单返回 `buildId=2026-05-04-1159`、`extensionVersion=0.1.76`；`npm run cloud:check`、`npm run cloud:audit-data-layer` 通过，线上探针同款样本为 `ai / ready / hide / high`。
