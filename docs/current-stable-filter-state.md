...
- 本地 Safari 扩展构建：
  ...
  - 扩展版本 `0.1.31`
  - App / Extension 版本 `1.0.31 (32)`
  - 2026-05-01 已接入 DeepSeek `deepseek-v4-flash` 作为回复区 AI 辅助层，并修复 AI 排队保护：只有本地规则先判定为可疑候选的回复才会进入 AI 队列，避免无脑调用模型。
- 云端 Cloudflare Worker：
  ...
  - 2026-05-01 已收紧回复区 AI 辅助层口径：正常成人话题、色情讨论、性教育/平台治理讨论不应仅因色情词被隐藏；基础层只压约见导流、联系方式、诈骗、木马/安装包、主页诱导、空洞钓鱼等高共识垃圾。
  - 控制台 AI 隐藏记录支持“恢复这条”，会把单条 AI 结果改成 `manual_allow/allow`，不作为公共规则反向训练。
