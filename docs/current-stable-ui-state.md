...
- `累计跳过无用内容`：后台直接下沉、后台学习库下沉、手动冲走、官方广告跳过的总数
- `后台直接下沉`：第一次交给后台新判断隐藏的回复
- `后台学习库下沉`：后台判过后被学习库记住、下次直接复用的隐藏
- `你手动冲走`：用户手动标记的内容
- `跳过官方广告`：主页和回复区官方广告跳过合计

旧的 `数据库历史命中`、`公共数据库规则`、`账号黑名单`、`本地规则下沉`、`恢复误判`、`累计涉及账号数`、`累计高频话术数`、主页/回复区广告分卡，不再作为主方格展示。旧来源如果仍有数据，应归并到 `后台学习库下沉` 或对应的手动/广告明细里。详情里要显示回复正文、账号、来源标签、简短原因、时间，并在有 X 链接时提供打开按钮。后台隐藏类仍保留“恢复误判”能力。
...
- 2026-05-02 AI 学习库改造后，详情入口固定为 `all_skipped`、`ai_direct`、`ai_memory`、`manual`、`ads`；2026-05-04 起用户可见文案统一叫“后台直接下沉 / 后台学习库下沉”，不再让用户区分 AI 后台和数据库后台。
- 2026-05-04 底部“查看列表”必须按当前详情页记住已下沉条目：X 展开底部或页面重排时，可能暂时把隐藏回复从 DOM 里卸掉，此时不能把底部状态改成“无下沉”、不能把按钮置灰、不能清空恢复列表。只有用户恢复、后台明确放过，或切换到其他页面 / 其他详情页时，才能清掉对应缓存。
- 2026-05-04 10:34 起，旧手动教学 / 历史命中在底部整理卡片里也显示为“后台学习库下沉”，不要再单独显示“手动记录下沉”。用户口径是：早期没有 AI 时，用户手动冲走是在当老师教数据库；这类旧记录属于后台学习库的一部分。除非确实有新的 `manual_hide` 事件，否则不能写成“你刚标记”或暗示用户刚刚点过。
...
- Safari 扩展 `BUILD_ID = 2026-05-04-0918`
...
2026-05-04 10:34 已替换本机 App 到 `BUILD_ID=2026-05-04-1034`，签名验证通过，App / Extension 版本为 `1.0.74 (75)`，manifest 为 `0.1.74`。本轮只修底部整理卡片的数据真实性文案：旧手动教学 / 历史命中统一显示为“后台学习库下沉”，`ai-pending` 单独计为“后台复审中”，不再把 pending 算进本机自动下沉；后台返回英文原因短句时，底部卡片会展示中文概括。真实 Safari 页 `https://x.com/fadai202202/status/2051011398134911475` 已验证 `build=2026-05-04-1034`、`stage=scan:done`、`articles=13`，底部显示 `后台已判断 6 条，后台自动下沉 1 条，后台学习库下沉 3 条`，不再出现“手动记录下沉”；展开、等待 4 秒、收起后 4 张底部卡片仍稳定存在，按钮未变灰。公网发布被 Cloudflare 登录失效挡住，公网 `/downloads/latest.json` 仍是上一版 `buildId=2026-05-04-0951`，等待重新登录后补发。

2026-05-04 09:57 已替换本机 App 到 `BUILD_ID=2026-05-04-0951`，签名验证通过，App / Extension 版本为 `1.0.71 (72)`，manifest 为 `0.1.71`。本轮只修底部查看列表稳定性：已下沉回复会按当前详情页缓存，X 短暂卸载回复 DOM 时不再误显示“无下沉”或清空恢复列表；恢复或后台明确放过时会清理缓存。公网已发布，Worker Version ID `7d573527-6054-4c19-935c-3fbafce92c01`，`/downloads/latest.json` 返回 `buildId=2026-05-04-0951`、`extensionVersion=0.1.71`。本轮没有改右栏 UI、筛选阈值、数据库结构或真实数据；真实 Safari X 标签页已读到新版，但当时 X 未加载出回复列表，所以尚无可见 `冲走` 按钮样本。

2026-05-04 09:38 已继续统一控制台和底部整理卡片的用户文案：官网控制台主方格改为“后台直接下沉 / 后台学习库下沉”，底部卡片和原因短句也统一使用“后台”。这次仍只改显示口径，不改统计来源、筛选阈值、数据库结构或真实数据。

2026-05-04 09:18 已更新底部整理卡片口径：不再显示“你刚标记下沉”，避免把历史/本机手动记录说成用户刚刚点击；AI 判断和数据库学习库统一显示为“后台”，例如“后台已判断 3 条（下沉 1 条）”。本轮没有改右栏 UI、筛选阈值或数据库结构，只修用户可见统计文案和来源分类。
